...
import { advanceRunners, applyPlayEvent, buildScoreByInning, undoLastPlay } from "@/lib/game-engine";
import { buildInningLabels, getCurrentLineupSlot, getPlayer, initialGameState, resultLabels, teams } from "@/lib/score-data";
...
export default function Home() {
  ...
  const recordPlay = () => {
    ...
    setGame((current) => {
      const batter = getCurrentLineupSlot(current);
      const transition = advanceRunners(current, batter.playerId, selectedResult);
      ...
      return applyPlayEvent(current, event);
    });
  };

  const undoPlay = () => {
    setGame((current) => undoLastPlay(current, initialGameState));
  };
