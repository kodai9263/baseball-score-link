...
export function MatchManager({ book, match, save, select, disabled }: Props) {
  ...
  return <section aria-label="試合の管理" className="mb-4 rounded-card border border-line bg-surface p-4">
    ...
    <details className="mt-3"><summary className="cursor-pointer text-sm font-bold text-primary">先攻・後攻の入れ替え</summary>
      ...
      <button type="button" disabled={disabled || creating || !!match.game.events.length || !!match.changes?.length} className="min-h-11 rounded-control border border-primary px-3 text-sm font-bold text-primary disabled:opacity-50" onClick={async () => {
        setError("");
