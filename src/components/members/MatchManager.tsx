...
type Props = { book: Scorebook; match: Match; save: (book: Scorebook) => Promise<boolean>; select: (id: string) => void; disabled: boolean; readOnly?: boolean };
export function MatchManager({ book, match, save, select, disabled, readOnly = false }: Props) {
  ...
  return <section aria-label="試合の管理" className="mb-4 rounded-card border border-line bg-surface p-4">
    <div className="flex flex-wrap items-end gap-3"><label className="min-w-0 flex-1 text-sm font-bold">表示する試合<select aria-label="表示する試合" className={input} disabled={disabled} value={match.id} onChange={event => select(event.target.value)}>{book.matches.map(item => <option key={item.id} value={item.id}>{item.date ?? "日付未設定"} ｜ {item.teams.away.name} vs {item.teams.home.name}</option>)}</select></label><button type="button" disabled={disabled || readOnly} className="min-h-11 rounded-control border border-primary px-3 text-sm font-bold text-primary disabled:opacity-50" onClick={() => setCreating(!creating)}>{creating ? "作成を閉じる" : "新しい試合"}</button></div>
    {!match.date ? <p className="mt-2 text-sm text-muted">この記録には試合日がありません。期間別の成績に含めるには日付を設定してください。</p> : null}
    <details className="mt-3"><summary className="cursor-pointer text-sm font-bold text-primary">先攻・後攻の入れ替え</summary>
      <p className="my-2 text-xs text-muted">チーム名と打順をまとめて入れ替えます。メンバーの所属や他の試合は変わりません。記録開始前だけ使えます。</p>
      <button type="button" disabled={disabled || readOnly || creating || !!match.game.events.length || !!match.changes?.length} className="min-h-11 rounded-control border border-primary px-3 text-sm font-bold text-primary disabled:opacity-50" onClick={async () => {
        setError("");
        try { await save({ ...book, matches: book.matches.map(item => item.id === match.id ? swapMatchSides(item) : item) }); }
        catch (cause) { setError(cause instanceof Error ? cause.message : "入れ替えできませんでした。"); }
      }}>先攻と後攻を入れ替える</button>
      {creating ? <p className="mt-2 text-xs text-muted">新しい試合の作成を閉じてから入れ替えてください。</p> : null}
      {match.game.events.length || match.changes?.length ? <p className="mt-2 text-xs text-muted">この試合は記録開始済みです。次の試合は「新しい試合」で作成後に入れ替えられます。</p> : null}
    </details>
    ...
    }}><fieldset disabled={disabled || readOnly} className="space-y-3"><label className="block max-w-xs text-sm">試合日<input required name="date" type="date" defaultValue={match.date ?? ""} className={input} /></label>
    ...
    }}><fieldset disabled={disabled || readOnly} className="space-y-4"><h2 className="font-bold">新しい試合を作成</h2><p className="text-xs text-muted">現在の試合を残したまま、新しいスコアを作ります。学年は試合日の年度から計算して保存します。</p>
