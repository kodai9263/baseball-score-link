...
import { currentLineups, changeLabels, defaultSources, positions } from "@/lib/lineup-changes";
...
export function LineupManager({ book, match, save, disabled }: { book: Scorebook; match: Match; save: (match: Match) => Promise<boolean>; disabled: boolean }) {
  ...
  const lineup = currentLineups(match)[team];
  ...
  const used = new Set([...Object.values(match.lineups).flat().map(slot => slot.playerId), ...(match.changes ?? []).map(change => change.incomingId)]);
  const bench = book.members.filter(member => member.team === (match.teamSources ?? defaultSources)[team] && !used.has(member.id));
  const orders = kind === "runner" ? lineup.filter(slot => Object.values(match.game.bases).includes(slot.playerId)) : lineup;
  ...
  return <details className="mb-4 rounded-card border border-line bg-surface p-4">
    ...
    <form className="mt-3 space-y-3" onSubmit={async event => {
      ...
      <label className="block text-sm">交代するチーム<select className={input} value={team} onChange={event => {
        const next = event.target.value as Half; setTeam(next); setKind(next === match.game.half ? "hitter" : "defense"); setOrder(1); setError("");
      ...
      <label className="block text-sm">交代の種類<select className={input} value={kind} onChange={event => {
        ...
        setOrder(next === "runner" ? lineup.find(slot => Object.values(match.game.bases).includes(slot.playerId))?.order ?? 1 : 1);
      ...
      {kind === "defense" || kind === "position" ? <label className="block text-sm">交代後の守備位置<select key={`${team}-${order}`} name="position" defaultValue={slot.position} className={input}>{positions.map(position => <option key={position}>{position}</option>)}</select></label> : null}
    ...
    {last ? <p role="status" className="mt-3 text-sm">直近の交代：{last.order}番 {getName(last.outgoingId)} → {getName(last.incomingId)}（{changeLabels[last.kind]}・{last.position}）。取消ボタンは交代も含め、最後の操作から戻します。</p> : null}
    {!!match.changes?.length && <ol aria-label="交代履歴" className="mt-3 space-y-1 text-xs text-muted">{match.changes.map(change => <li key={change.id}>{change.beforePlay}プレー記録後・{change.team === "top" ? "先攻" : "後攻"}{change.order}番 {getName(change.outgoingId)} → {getName(change.incomingId)}／{changeLabels[change.kind]}・{change.position}</li>)}</ol>}
  </details>;
}
