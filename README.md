...
メンバーから通算・学年別・期間別の成績を確認でき、JSONファイルでバックアップと復元ができます。初回のチーム・選手登録、メール確認コードによるログイン、招待、管理者・記録者・閲覧者の権限、チーム別クラウド保存に対応しています。

Supabase未設定時は端末保存で動作します。クラウド運用にはDB・メール認証・環境変数の設定が必要です。[本番接続手順](docs/cloud-setup.md)を参照してください。本番サービスへの接続・公開はまだ行っていません。
